...
<!-- 			</select></li> -->
	...
	<div class="container">
	<c:if test="${studyFilter.size()>0 }">
		<!-- 스터디 목록 출력 -->
		<c:forEach items="${studyFilter }" var="study" begin="0" end="${studyFilter.size()-1 }">
			<ul class="search_list">
				<!-- 앵커 누르면 detail페이지로 연결 -->
				<li><a href="/study/detail?study_no=${study.study_no }">
						<div class="left">
							<div class="div1">
								<img src="/cos/upload/${study.file_storedname }">
								<hr>
							</div>
						</div>
						<div class="right">
							...
							<p class="p">스터디 시작일 : ${study.study_opendate }</p>
							<hr>
						</div>
		...
		<c:if test="${studyFilter.size() eq 0 }">
